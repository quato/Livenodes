...
/* *** BEGIN OBFUSCATION MAGIC - LivenodesCoin **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat LivenodesCoin and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 LivenodesCoin per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100LivenodesCoin+1 ( bit on if present )
    // bit 1 - 10LivenodesCoin+1
    // bit 2 - 1LivenodesCoin+1
    // bit 3 - .1LivenodesCoin+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("livenodes-obfuscation");
