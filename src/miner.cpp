...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LivenodesCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("LivenodesCoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("LivenodesCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("livenodes-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || !masternodeSync.IsSynced()) {
        ...
        LogPrintf("Running LivenodesCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
