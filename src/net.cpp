...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "LivenodesCoin " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. LivenodesCoin Core is probably already running."), addrBind.ToString());
        else
...
void RelayInv(CInv& inv)
{
    LOCK(cs_vNodes);
    for (CNode* pnode : vNodes){
        if(inv.IsMasterNodeType())continue;
