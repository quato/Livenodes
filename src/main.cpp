...
#if defined(NDEBUG)
#error "LivenodesCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulivenodes) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
  // anti instamine
    ...
    if ( nHeight == 0 ) {    // premine
      ...
      nSubsidy = 9 * COIN;
    } else if (nHeight > 20000 && nHeight <= 35000) {
      nSubsidy = 8 * COIN;
    } else if (nHeight > 35000 && nHeight <= 50000) {
      nSubsidy = 7 * COIN;
    } else if (nHeight > 50000 && nHeight <= 70000) {
      nSubsidy = 6 * COIN;
    } else if (nHeight > 70000 && nHeight <= 90000) {
      nSubsidy = 5 * COIN;
...
{
    RenameThread("livenodes-scriptch");
